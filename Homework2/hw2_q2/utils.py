...
def reshape_tensor_dataset(ds):
    """
    Reshape a dataset of RNA sequences for use with 2D convolutional neural networks.

    This function takes a dataset of tuples (x, y, mask), where:
        - x is a tensor of shape (seq_len, 4) representing one-hot encoded RNA sequences,
        - y is the target binding affinity,
        - mask indicates valid positions in y (for handling padded sequences).

    Each x tensor is unsqueezed along a new channel dimension to produce shape 
    (1, seq_len, 4), making it compatible with Conv2D layers. All x, y, and mask 
    tensors are stacked along the batch dimension and returned as a TensorDataset.

    Args:
        ds (Iterable[Tuple[Tensor, Tensor, Tensor]]): Input dataset of RNA sequences.

    Returns:
        TensorDataset: A dataset of tuples (x, y, mask), where:
            - x has shape (N, 1, seq_len, 4)
            - y has shape (N,)
            - mask has shape (N,)
    """
